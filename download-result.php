...
$qery = "SELECT   students.StudentName,students.RollId,students.RegDate,students.StudentId,students.Status,classes.ClassName,classes.Section from students join classes on classes.id=students.ClassId where students.RollId=? and students.ClassId=?";
...
<?php                                              
 ...
 $query ="select t.StudentName,t.RollId,t.ClassId,t.marks,SubjectId,subjects.SubjectName from (select sts.StudentName,sts.RollId,sts.ClassId,tr.marks,SubjectId from students as sts join  result as tr on tr.StudentId=sts.StudentId) as t join subjects on subjects.id=t.SubjectId where (t.RollId=? and t.ClassId=?)";
